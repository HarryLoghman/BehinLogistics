...
namespace RailSiteDataGrabber.rwmms.model
{
    public class webpage : IDisposable
    {
        ...
        {
            return this.fnc_save(false, false, false);
        }

        public bool fnc_save(bool getDetail, bool download, bool startFromLastPage)
        {
            int i;
            for (i = 0; i <= this.prp_tables.Count - 1; i++)
            {
                if (!this.prp_tables[i].fnc_save(getDetail, download, startFromLastPage))
                    return false;
            }
            return true;
        }
