...
namespace RailSiteDataGrabber.rwmms.model
{
    public class htmlTable : DataTable
    {
        ...
        public htmlTable(string tableName, webpage page, string columnIdName, By gridBy, By comboBy) : base(tableName)
        {
            this.prp_webpage = page;

            this.v_handleRowChange = true;
            this.Columns.CollectionChanged += Columns_CollectionChanged;
            this.prp_gridBy = gridBy;
            this.prp_comboBy = comboBy;
            this.prp_columnIdName = columnIdName;
        }

        #region properties and variables
        public By prp_gridBy { get; set; }
        public By prp_comboBy { get; set; }
        /// <summary>
        ...
        private void Columns_CollectionChanged(object sender, CollectionChangeEventArgs e)
        {
            if (e.Action == CollectionChangeAction.Add)
            {
                ...
                if (e.Element is htmlControlCombo)
                {
                    ...
                    if (this.prp_comboBy == null)
                    {
                        throw new Exception("prp_comboBy is not specified");
                    }
                ...
                {
                    if (this.prp_gridBy == null)
                    {
                        throw new Exception("prp_gridBy is not specified");
                    }
                    for (i = 0; i <= this.Columns.Count - 2; i++)
                    {
        ...
        public virtual bool fnc_save()
        {
            return this.fnc_save(false, false, 1);
        }
        public virtual bool fnc_save(bool getDetail, bool download, bool startFromLastPage)
        {
            ...
            return this.fnc_save(getDetail, download, startPageIndex);

        }
        public virtual bool fnc_save(bool getDetail, bool download, int startPageIndex)
        {
            ...
            if (this.prp_gridBy != null)
            {
                if (!this.fnc_saveGrid(getDetail, download, startPageIndex))
                    return false;
            }
            else if (this.prp_comboBy != null)
            {
                IWebElement combo = this.prp_webBrowser.FindElement(this.prp_comboBy);
        ...
        private bool fnc_saveGrid(bool getDetail, bool download, int startPageIndex)
        {
            int i;
            int pageIndex;
            IWebElement grid = this.prp_webBrowser.FindElement(this.prp_gridBy);
            int pageCount;
            ...
            {
                pageCount = htmlGrid.fnc_getPageCount(this.prp_webBrowser, this.prp_webpage.prp_url, grid, this.fnc_getRowCountHtmlElement(), null, this.prp_pageRowCount, false);
            }
            ...
            for (pageIndex = startPageIndex; pageIndex <= pageCount && !stopFetch; pageIndex++)
            {
                if (pageIndex > 1)
                {
                    pageIndex = htmlGrid.fnc_gotoPage(this.prp_webBrowser, this.prp_webpage.prp_url, this.prp_gridBy, this.fnc_getRowCountHtmlElement(), pageIndex, pageCount, false
                        , this.prp_rowCountLabelRegexPattern, this.prp_pageRowCount, true);
                    if (pageIndex == -1) return false;
                    grid = this.prp_webBrowser.FindElement(this.prp_gridBy);
                }
