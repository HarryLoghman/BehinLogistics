...
namespace RailSiteDataGrabber.rwmms.model
{
    ...
    public class htmlControlDownload : htmlControl
    {
        ...
        protected string fnc_downloadFile(IWebElement elementDownload, string fileNameWithoutExtension)
        {
            ...
            SharedFunctions.sb_waitForReady(((htmlTable)this.Table).prp_webBrowser);
            FileInfo myFile;
            do
            {
    ...
    public class htmlControlComboSelectedItem : htmlControl
    {
        ...
        public override object fnc_getColumnValue()
        {
            ...
            if (element != null)
            {
                ...
                {
                    Program.logs.Error("htmlControlComboSelectedItem(" + this.prp_controlBy.ToString() + "):getColumnValu", ex);
                }
            }
