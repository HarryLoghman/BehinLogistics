    ...
    class billOfLadingsTracking
    {
        ...
        private void sb_saveToDB(DataTable dt, int cycleNumber)
        {
            ...
            using (var entityLogistic = new Model.logisticEntities())
            {
                ...
                {
                    entry_raiBillOfLadingsTracking = new Model.raiBillOfLadingsTracking();

                    entry_raiBillOfLadingsTracking.billOfLadingId = (Functions.IsNull(dt.Rows[i][billOfLadingsTrackingDataTable.fld_billOfLadingNo]) ? null : PWS0.trainBarryBillOfLadings.fnc_getBillOfLadingId(long.Parse(dt.Rows[i][billOfLadingsTrackingDataTable.fld_billOfLadingNo].ToString()), true));
                    entry_raiBillOfLadingsTracking.currentStationId = (Functions.IsNull(dt.Rows[i][billOfLadingsTrackingDataTable.fld_currentStationName]) ? null : PWS0.stations.fnc_getStationId(null, dt.Rows[i][billOfLadingsTrackingDataTable.fld_currentStationName].ToString(), true));
                    entry_raiBillOfLadingsTracking.CycleNumber = cycleNumber;
                    entry_raiBillOfLadingsTracking.destinationStationId = (Functions.IsNull(dt.Rows[i][billOfLadingsTrackingDataTable.fld_destinationStationName]) ? null : PWS0.stations.fnc_getStationId(null, dt.Rows[i][billOfLadingsTrackingDataTable.fld_destinationStationName].ToString(), true));
                    ...
                    entry_raiBillOfLadingsTracking.sourceStationId = (Functions.IsNull(dt.Rows[i][billOfLadingsTrackingDataTable.fld_sourceStationName]) ? null : PWS0.stations.fnc_getStationId(null, dt.Rows[i][billOfLadingsTrackingDataTable.fld_sourceStationName].ToString(), true));
                    entry_raiBillOfLadingsTracking.trainId = (Functions.IsNull(dt.Rows[i][billOfLadingsTrackingDataTable.fld_trainNo]) ? null : PWS0.trainBarry.fnc_getTrainIdFromDB(int.Parse(dt.Rows[i][billOfLadingsTrackingDataTable.fld_trainNo].ToString()), enteranceDateTime));
                    entry_raiBillOfLadingsTracking.wagonId = (Functions.IsNull(dt.Rows[i][billOfLadingsTrackingDataTable.fld_wagonNo]) ? null : PWS0.trainBarryWagons.fnc_getWagonIdCheckWithControlNo(long.Parse(dt.Rows[i][billOfLadingsTrackingDataTable.fld_wagonNo].ToString()), true));
