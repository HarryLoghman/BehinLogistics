...
{
    public class wagonInfoTrackingDetail
    {
        ...
        private void sb_saveToDB(DataTable dt, int cycleNumber)
        {
            ...
            using (var entityLogistic = new logisticEntities())
            {
                for (i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    ...
                    entry_raiWagon_Info_Tracking.currentStationId = (Functions.IsNull(this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_currentStationName]) ? null : stations.fnc_getStationId(null, this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_currentStationName].ToString(), true));
                    entry_raiWagon_Info_Tracking.CycleNumber = cycleNumber;
                    entry_raiWagon_Info_Tracking.destinationStationId = (Functions.IsNull(this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_destinationStationName]) ? null : stations.fnc_getStationId(null, this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_destinationStationName].ToString(), true));
                    ...
                    entry_raiWagon_Info_Tracking.sourceStationId = (Functions.IsNull(this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_sourceStationName]) ? null : stations.fnc_getStationId(null, this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_sourceStationName].ToString(), true));
                    entry_raiWagon_Info_Tracking.trainId = (Functions.IsNull(this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_trainNo]) ? null : trainBarry.fnc_getTrainIdFromDB(int.Parse(this.v_dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_trainNo].ToString()), enteranceDateTime));

                    if (!Functions.IsNull(dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_wagonNo]))
                        entry_raiWagon_Info_Tracking.wagonId = trainBarryWagons.fnc_getWagonIdCheckWithControlNo(long.Parse(dt.Rows[i][wagonInfoTrackingDetailDataTable.fld_wagonNo].ToString()), true);
