...
namespace CustomerLibrary.Rai
{
    public class trainPassengersLastStatus
    {
        ...
        private void sb_saveToDB(DataTable dt, int cycleNumber)
        {
            ...
            raiTrainPassengersLastStatu entry_raiTrainPassengersLastStatus;
            using (var entityLogistic = new  logisticEntities())
            {
                for (i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    entry_raiTrainPassengersLastStatus = new  raiTrainPassengersLastStatu();

                    entry_raiTrainPassengersLastStatus.areaId = (Functions.IsNull(dt.Rows[i][trainPassengersLastStatusDataTable.fld_areaName]) ? null :  areas.fnc_getAreaId(dt.Rows[i][trainPassengersLastStatusDataTable.fld_areaName].ToString(), false, true));
                    entry_raiTrainPassengersLastStatus.currentStationId = (Functions.IsNull(dt.Rows[i][trainPassengersLastStatusDataTable.fld_currentStationName]) ? null :  stations.fnc_getStationId(null, dt.Rows[i][trainPassengersLastStatusDataTable.fld_currentStationName].ToString(), true));
                    entry_raiTrainPassengersLastStatus.CycleNumber = cycleNumber;
                    entry_raiTrainPassengersLastStatus.destinationStationId = (Functions.IsNull(dt.Rows[i][trainPassengersLastStatusDataTable.fld_destinationStationName]) ? null :  stations.fnc_getStationId(null, dt.Rows[i][trainPassengersLastStatusDataTable.fld_destinationStationName].ToString(), true));
                    ...
                    entry_raiTrainPassengersLastStatus.sourceStationId = (Functions.IsNull(dt.Rows[i][trainPassengersLastStatusDataTable.fld_sourceStationName]) ? null : stations.fnc_getStationId(null, dt.Rows[i][trainPassengersLastStatusDataTable.fld_sourceStationName].ToString(), true));
                    entry_raiTrainPassengersLastStatus.trainId = (Functions.IsNull(dt.Rows[i][trainPassengersLastStatusDataTable.fld_trainNo]) ? null :  trainBarry.fnc_getTrainIdFromDB(int.Parse(dt.Rows[i][trainPassengersLastStatusDataTable.fld_trainNo].ToString()), enteranceDateTime));
