...
{
    public class wagonInfoTrackingResult
    {
            ...
            using (var entityLogistic = new logisticEntities())
            {
                for (i = 0; i <= dt.Rows.Count - 1; i++)
                {
                    ...
                    if (!Functions.IsNull(dt.Rows[i][wagonInfoTrackingResultDataTable.fld_currentStationName]))
                        entry_raiWagon_Info_Tracking.currentStationId = stations.fnc_getStationId(null, dt.Rows[i][wagonInfoTrackingResultDataTable.fld_currentStationName].ToString(), true);
                    ...
                    if (!Functions.IsNull(dt.Rows[i][wagonInfoTrackingResultDataTable.fld_destinationStationName]))
                        entry_raiWagon_Info_Tracking.destinationStationId = stations.fnc_getStationId(null, dt.Rows[i][wagonInfoTrackingResultDataTable.fld_destinationStationName].ToString(), true);
                    ...
                    if (!Functions.IsNull(dt.Rows[i][wagonInfoTrackingResultDataTable.fld_barType]))
                        entry_raiWagon_Info_Tracking.goodsID = goods.fnc_getGoodsId(dt.Rows[i][wagonInfoTrackingResultDataTable.fld_barType].ToString()
                            , false, true);
                    ...
                    if (!Functions.IsNull(dt.Rows[i][wagonInfoTrackingResultDataTable.fld_sourceStationName]))
                        entry_raiWagon_Info_Tracking.sourceStationId = stations.fnc_getStationId(null, dt.Rows[i][wagonInfoTrackingResultDataTable.fld_sourceStationName].ToString(), true);
                    if (!Functions.IsNull(dt.Rows[i][wagonInfoTrackingResultDataTable.fld_wagonNo]))
                        entry_raiWagon_Info_Tracking.wagonId = trainBarryWagons.fnc_getWagonIdCheckWithControlNo(long.Parse(dt.Rows[i][wagonInfoTrackingResultDataTable.fld_wagonNo].ToString()), true);
